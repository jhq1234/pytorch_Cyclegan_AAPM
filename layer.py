...
class DECBR2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True, norm='norm', relu=0.0):
        super().__init__()
        layers = []
...
class PixelShuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, C, H // ry, ry, W // rx, rx) #6개의 axis 정의

        # 6개의 axis permutation 진행행
...
class PixelUnshuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, C // (ry * rx), ry, rx, H, W)#6개의 axis 정의
