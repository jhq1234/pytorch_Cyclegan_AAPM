...
class CycleGAN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.tanh(x) # 트레이닝이 수월하게 진행되기 위해 추가.
...
class Pix2Pix(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm="bnorm"):
        ...
        self.enc2 = CBR2d(in_channels=1 * nker, out_channels=2 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc3 = CBR2d(in_channels=2 * nker, out_channels=4 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc4 = CBR2d(in_channels=4 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc5 = CBR2d(in_channels=8 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc6 = CBR2d(in_channels=8 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc7 = CBR2d(in_channels=8 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)
        self.enc8 = CBR2d(in_channels=8 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                          norm=norm, relu=0.2, stride=2)

        self.dec1 = DECBR2d(in_channels=8 * nker, out_channels=8 * nker, kernel_size=4, padding=1,
                            norm=norm, relu=0.0, stride=2)
        self.drop1 = nn.Dropout2d(0.5)

        self.dec2 = DECBR2d(in_channels=2 * 8 * nker, out_channels=8 * nker,
                            kernel_size=4, padding=1,
                            norm=norm, relu=0.0, stride=2)
        self.drop2 = nn.Dropout2d(0.5)

        self.dec3 = DECBR2d(in_channels=2 * 8 * nker, out_channels=8 * nker,
                            kernel_size=4, padding=1,
                            norm=norm, relu=0.0, stride=2)
        self.drop3 = nn.Dropout2d(0.5)

        self.dec4 = DECBR2d(in_channels=2 * 8 * nker, out_channels=8 * nker,
                          kernel_size=4, padding=1,
                          norm=norm, relu=0.0, stride=2)

        self.dec5 = DECBR2d(in_channels=2 * 8 * nker, out_channels=4 * nker,
                          kernel_size=4, padding=1,
                          norm=norm, relu=0.0, stride=2)

        self.dec6 = DECBR2d(in_channels=2 * 4 * nker, out_channels=2 * nker,
                          kernel_size=4, padding=1,
                          norm=norm, relu=0.0, stride=2)

        self.dec7 = DECBR2d(in_channels=2 * 2 * nker, out_channels=1 * nker,
                          kernel_size=4, padding=1,
                          norm=norm, relu=0.0, stride=2)

        self.dec8 = DECBR2d(in_channels=2 * 1 * nker, out_channels=out_channels,
                          kernel_size=4, padding=1,
...
class Discriminator(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm="bnorm"):
        ...
        self.enc1 = CBR2d(in_channels=in_channels, out_channels=1 * nker, kernel_size=4, stride=2,
                          padding=1, norm=None, relu=0.2, bias=False)
        self.enc2 = CBR2d(in_channels=1 * nker, out_channels=2 * nker, kernel_size=4, stride=2,
                          padding=1, norm=norm, relu=0.2, bias=False)
        self.enc3 = CBR2d(in_channels=2 * nker, out_channels=4 * nker, kernel_size=4, stride=2,
                          padding=1, norm=norm, relu=0.2, bias=False)
        self.enc4 = CBR2d(in_channels=4 * nker, out_channels=8 * nker, kernel_size=4, stride=2,
                          padding=1, norm=norm, relu=0.2, bias=False)
        self.enc5 = CBR2d(in_channels=8 * nker, out_channels=out_channels, kernel_size=4, stride=2,
                          padding=1, norm=None, relu=None, bias=False)
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels,nker=64, norm='norm', learning_type='plain'):
        ...
        self.dec1_0 = CBR2d(in_channels=1 * nker, out_channels=out_channels, norm=norm, relu=None)
